...
Bundled `items.yml` and `stats.yml` use `config-version: 3`. Migration runs before normal configuration loading and is version-aware: V1-to-V3 preserves administrator-owned legacy configuration, including deliberately removed legacy profiles, while adding only reserved schema profiles introduced after V1; V2-to-V3 adds missing reserved parents and performs only safe legacy-parent rewrites without restoring deliberately removed fields. Backups use the source version (`.v1.bak` or `.v2.bak`) before replacement. A parse or migration failure prevents partial startup, and V3 files are not migrated again.
