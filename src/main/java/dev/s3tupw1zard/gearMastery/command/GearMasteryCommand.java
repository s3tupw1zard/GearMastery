...
public final class GearMasteryCommand implements CommandExecutor, TabCompleter {
    ...
    private boolean reload(final CommandSender sender) {
        if (!sender.hasPermission("gearmastery.admin.reload")) return denied(sender);
        final boolean reloaded = configurations.reload();
        if (reloaded) org.bukkit.Bukkit.getOnlinePlayers().forEach(progression::synchronizePlayer);
        sender.sendMessage(reloaded ? "GearMastery configuration reloaded." : "Reload failed; previous configuration remains active."); return true;
    }
