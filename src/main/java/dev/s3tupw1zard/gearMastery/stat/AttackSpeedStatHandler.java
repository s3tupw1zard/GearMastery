...
public final class AttackSpeedStatHandler implements StatHandler {
    ...
    @Override public void apply(final StatApplicationContext context) {
        ...
        final double effectiveBaseline = storedBaseline.orElseGet(() -> {
            final double captured = effectiveBaseline(existing, modifierKey, GearModifierKeys.legacy(StatType.ATTACK_SPEED), slot); return captured;
        });
        ...
        existing.modifiers().stream().filter(entry -> !GearModifierKeys.owns(entry.modifier().getKey(), StatType.ATTACK_SPEED))
